...
import ShoulderMeasurement from './ShoulderMeasurement';

function ShoulderInput(props) {
  const [ filePath, setFilePath ] = useState();
  const [ uploaded, setUploaded ] = useState(false);
  const [ side, setSide ] = useState('right');
  const { hamangle, hamconfidence } = props.location.state
  ...
  return (
    <div>
      {(uploaded) ? (<ShoulderMeasurement path={filePath.path} type={filePath.type} side={side} hamAngle={hamangle} hamConfidence={hamconfidence} />) : (<div><div>Import photo or video of shoulder stretch</div>
